...
class BlockStructure(object):
    ...
    def _add_block(block_relations, usage_key):
        ...
        # Since block_relations is a DefaultDict, the usage_key will
        # automatically be added by virtue of accessing it.
        _ = block_relations[usage_key]
...
class BlockStructureBlockData(BlockStructure):
    ...
    def remove_transformer_block_data(self, usage_key, transformer, key):
        """
        Deletes the given transformer's data dictionary value for the
        given key for the block identified by the given usage_key.
        ...
        Arguments:
            ...
            key (string) - The key to the transformer's dictionary entry
...
class BlockStructureXBlockData(BlockStructureBlockData):
    ...
    def _set_xblock_field(self, usage_key, xblock, field_name):
        ...
        if hasattr(xblock, field_name):
            self._block_data_map[usage_key]._xblock_fields[field_name] = getattr(xblock, field_name)
