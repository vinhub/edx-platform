...
class BlockStructureFactory(object):
    ...
    def serialize_to_cache(cls, block_structure, cache):
        ...
        The key in the cache is 'root.key.<root_block_key>'.
    ...
    @classmethod
    def create_from_cache(cls, root_block_key, cache, transformers):
        """
        Deserializes and returns the block structure starting at
        root_block_key from the given cache, if it's found in the cache.

        The given root_block_key must equate the root_block_key
        ...
        Returns:
            ...
            at root_block_key, if found in the cache.

            NoneType - If the root_block_key is not found in the cache
            or if the cached data is outdated for one or more of the
            given transformers.
        """

        # Find root_block_key in the cache.
    ...
    def remove_from_cache(cls, root_block_key, cache):
        """
        Removes the block structure for the given root_block_key
